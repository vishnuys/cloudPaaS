...
def job_accept_cb(ch, method, properties, body):
	...
	with open(job_instance.filepath) as csvfile:
		...
		for row in reader:
			...
			resp = {'jobid': jobid,'finalop': json.loads(job_instance.service_order)[-1], 'val': row[job_instance.colname]}
			chan.basic_publish(
        		exchange='',
			...
			connection =pika.BlockingConnection(pika.ConnectionParameters('localhost'))
			chan = connection.channel()		
			resp = {'jobid': jobid, 'finalop': json.loads(job_instance.service_order)[-1], 'val': 'FINAL'}
